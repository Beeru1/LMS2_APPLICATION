	...
	**/
	...
	public ActionForward init(
		...
		throws Exception {
			...
			UserMstr sessionUserBean = (UserMstr) request.getSession().getAttribute("USER_INFO");
	...
	public ActionForward showAlertMessages(
		...
		throws Exception {
			...
			if(alertList!=null ){
				////logger.info("Alert size "+ alertList.size());
				if(alertList.size()>0){
	...
	public ActionForward sessionExpiryAlert(
			...
			//	//logger.info("SessionTime "+sessionUserBean.getSessionExpiry());
			...
			//	//logger.info("Session Expiry Message : "+sessionExpiryMessage);
	...
	throws Exception {
		...
		long token = -1; 
		String sessionExpiryMessage="none";
		//logger.info("Timeleft :"+timeLeft + "   Inactive Interval "+maxT+"    craetion time : "+startT+"    backupT : "+backupT );
		if(timeLeft<=0){
