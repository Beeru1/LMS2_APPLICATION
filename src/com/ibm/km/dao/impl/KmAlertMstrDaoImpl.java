...
public class KmAlertMstrDaoImpl implements KmAlertMstrDao {
 ...
 protected static final String SQL_INSERT ="INSERT INTO LMS.KM_ALERT_MSTR (MESSAGE, MESSAGE_ID, CIRCLE_ID,CREATED_DT, CREATED_BY, UPDATED_DT, UPDATED_BY, STATUS, ALERT_SOURCE) VALUES (?, ? , ?,  current timestamp, ?, current timestamp,?, ? ,?)";
 
 protected static final String SQL_ALERT_MESSEGE ="SELECT MESSAGE,CREATED_DT AS CREATED_DT FROM LMS.KM_ALERT_MSTR  where CREATED_DT =(SELECT max(CREATED_DT) FROM LMS.KM_ALERT_MSTR where CIRCLE_ID = ? and STATUS = 'A') ";
 
 protected StringBuffer SQL_VIEW = new StringBuffer("Select * from LMS.KM_ALERT_MSTR ");

 protected static final String SQL_DELETE ="delete from LMS.KM_ALERT_MSTR where MESSAGE_ID=?";
 
 protected static final String SQL_UPDATE = "update LMS.KM_ALERT_MSTR set (MESSAGE,UPDATED_DT,UPDATED_BY)=(?,current timestamp,?) where CREATED_BY=? ";
 
 protected StringBuffer UPDATE_ALERT_FLAG = new StringBuffer("update LMS.KM_USER_MSTR set USER_ALERTS =? ");
 
// protected String sqlupdateAlertId = "select ALERT_ID,USER_ALERTS,TIMESTAMPDIFF(4, CAST(SESSION_EXPIRY_TIME-CURRENT TIMESTAMP AS CHAR(22))) AS MINUTES from LMS.KM_USER_MSTR where USER_ID=? ";
  
 protected String sqlupdateAlertId = "select ALERT_ID,USER_ALERTS from LMS.KM_USER_MSTR where USER_ID=? ";
  /*km phase2 edit alert & alerthistory report*/  
 protected static final String SQL_GET_ALERT="select date(LMS.KM_ALERT_MSTR.CREATED_DT) as CREATED_DATE,LMS.KM_ALERT_MSTR.CIRCLE_ID,Time(LMS.KM_ALERT_MSTR.CREATED_DT)as CREATION_TIME ,LMS.KM_ALERT_MSTR.MESSAGE,LMS.KM_USER_MSTR.USER_LOGIN_ID as ADMIN_ID,LMS.KM_ALERT_MSTR.ALERT_SOURCE from LMS.KM_ALERT_MSTR  inner join LMS.KM_USER_MSTR on LMS.KM_ALERT_MSTR.CREATED_BY=LMS.KM_USER_MSTR.USER_ID where LMS.KM_ALERT_MSTR.CIRCLE_ID = ? ";
 
 protected static final StringBuffer SQL_GET_LATEST_ALERTS=new StringBuffer("select date(LMS.KM_ALERT_MSTR.CREATED_DT) as CREATED_DATE,LMS.KM_ALERT_MSTR.MESSAGE_ID,Time(LMS.KM_ALERT_MSTR.CREATED_DT)as CREATION_TIME ,LMS.KM_ALERT_MSTR.MESSAGE,LMS.KM_ELEMENT_MSTR.ELEMENT_NAME as CIRCLE_ID,LMS.KM_ALERT_MSTR.ALERT_SOURCE from LMS.KM_ALERT_MSTR  inner join LMS.KM_ELEMENT_MSTR on  LMS.KM_ALERT_MSTR.CIRCLE_ID=LMS.KM_ELEMENT_MSTR.ELEMENT_ID ");
 
 protected static final String ALERT_UPDATE ="update LMS.KM_ALERT_MSTR set (MESSAGE,UPDATED_DT)=(?,current timestamp) where MESSAGE_ID=?";
  
 // protected String sqlupdateAlertId="select * from LMS.KM_USER_MSTR ";
 /* (non-Javadoc)
  * @see com.ibm.km.dao.KmAlertMstrDao#insert(com.ibm.km.dto.KmAlertMstr)
 * inser a record for a Alert in the table KM.Alert_MSTR. Insert values like message, start date, end date etc
 *  */

 public int insertNew(KmAlertMstr dto) throws LMSException {
 ...
 public int insert(KmAlertMstr dto) throws LMSException {
	  ...
	  try {
	   con = DBConnection.getDBConnection();
	   int messageId;
	   ps=con.prepareStatement("SELECT NEXTVAL FOR LMS.KM_ALERT_ID_SEQ AS MESSAGE_ID  FROM SYSIBM.SYSDUMMY1 ");
	   rs=ps.executeQuery();
	   if(rs.next()){
	   ...
	   //***** Code added (by Kundan) to resolve alert issue - (max 10 alerts should be there in column)*****//
		   ...
		   ps=con.prepareStatement("select MESSAGE_ID from LMS.KM_ALERT_MSTR where CIRCLE_ID = ?   and  minute (current timestamp  - CREATED_DT) < "+maxMinutesAlertsExpires+"   order by created_dt desc fetch first "+ maxNoOfAlerts +" rows only");
	   ...
	   ps = con.prepareStatement("UPDATE LMS.KM_USER_MSTR set USER_ALERTS =? , ALERT_ID = '"+ alertIds+"' where ELEMENT_ID = ? and KM_ACTOR_ID IN (?,?) and status = 'A'  with ur  ");	   
	   ps.setString(1, "N");
 ...
 public String deleteAlert(KmAlertMstr dto) throws LMSException {
  logger.info("Entered delete Alert for table LMS.KM_Alert_MSTR");
  Connection con = null;
  PreparedStatement ps = null;
  ResultSet rs = null;
  ...
  */
 public ArrayList getAlertListNew(KmAlertMstr dto) throws LMSException {
  ...
  //  alerts.add(alertList);
    ...
    String updateAlertId =
     "update LMS.KM_USER_MSTR set USER_ALERTS =?,ALERT_ID=? where USER_ID=? ";
 ...
 public ArrayList getAlertList(KmAlertMstr dto) throws LMSException {
  ...
  try {
   ...
   ps = con.prepareStatement("select ALERT_ID from LMS.KM_USER_MSTR where USER_ID=? AND USER_ALERTS = 'N' with ur");
   ps.setInt(1, dto.getUserId());
   rs = ps.executeQuery();
	 		...
	 		try{
			  ...
			  int size=alertIds.size();
			    ...
			    if (size > maxNoOfAlerts ) {
			    	...
			    	/*//logger.info("1st element removed ? "+alertIds.remove(0));
			    	StringBuffer alerts=new StringBuffer("");
			    	for(int j=0;j<10;j++){
...
private String alertInsert(String alertfrmUser, List newAlertIds) {
 ...
 public ArrayList getAlertHistoryReport(String circleId,String timeInterval) throws LMSException{
		 ...
		 try {
			...
			query.append(" AND(current timestamp - LMS.KM_ALERT_MSTR.CREATED_DT )< "+timeInterval+"  order by LMS.KM_ALERT_MSTR.CREATED_DT DESC ");
 ...
 public ArrayList getAlerts(String circleId,String time) throws LMSException{
			...
			//String time=bundle.getString("alert.time");
			 try {
				 ...
				 sql.append(
				 " where (date(LMS.KM_ALERT_MSTR.CREATED_DT))=(current date)  and LMS.KM_ALERT_MSTR.CIRCLE_ID=? and (current timestamp - LMS.KM_ALERT_MSTR.CREATED_DT )< ").append(time);
				 logger.info(sql.toString());
