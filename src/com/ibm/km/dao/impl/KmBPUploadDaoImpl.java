...
public class KmBPUploadDaoImpl implements KmBPUploadDao{
	...
	.getLogger(KmBPUploadDaoImpl.class.toString());
	...
	public ArrayList<KmBPUploadDto> getHeaders() throws DAOException{
		...
		ResultSet rs = null;
		String query = "Select HEADER_ID,HEADER_NAME,CATEGORY_ID from LMS.KM_BP_HEADER_MSTR";
		...
		finally
		{
			...
			{
				logger.severe("DAOException occured in getHeaders" + "Exception Message: " + e.getMessage());
	...
	public int insertBPData(KmBPUploadDto dto) throws DAOException{
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query="insert into LMS.KM_BP_DATA(TOPIC,HEADER,CONTENT,PATH,FROM_DT,TO_DT,DOCUMENT_ID,CIRCLE_ID) values(?,?,?,?,?,?,?,?)";
		try {
			   conn = DBConnection.getDBConnection();
			   pstmt = conn.prepareStatement(query);
			   ArrayList<String> headerList = dto.getHeaderList(); 
			   ArrayList<String> contentList  = dto.getContentList();
			   //logger.info("headerList in daoimpl="+headerList.get(0));
			   //logger.info("contentList in daoimpl="+contentList.get(0));
			   //logger.info(dto.getFromDate());
			   //logger.info(dto.getToDate());
			   ...
			   pstmt.executeBatch();
			   ...
			   finally
				{
					...
					{
						logger.severe("DAOException occured in insertBPData" + "Exception Message: " + e.getMessage());
						throw new LMSException(e.getMessage(),e);
	...
	public ArrayList<KmBPUploadDto> viewBPDetails(KmBPUploadDto dto) throws DAOException{
		...
		finally
		{
			...
			{
				logger.severe("DAOException occured in viewBPDetails" + "Exception Message: " + e.getMessage());
				throw new LMSException(e.getMessage(),e);
			}
		}	
		return dataList;
	...
	public int deleteBPDetails(KmBPUploadDto dto) throws DAOException{
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query="DELETE FROM LMS.KM_BP_DATA where DOCUMENT_ID=?";
		...
		finally
		{
			...
			{
				logger.severe("DAOException occured in deleteBPDetails" + "Exception Message: " + e.getMessage());
				throw new LMSException(e.getMessage(),e);
			}
		}	
	...
	public ArrayList<String> getDocumentId(String searchKey) throws DAOException{
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		//String query="Select distinct document_id from LMS.KM_BP_DATA where upper(?) in(upper(CONTENT),upper(TOPIC))";
		String query="Select distinct document_id from LMS.KM_BP_DATA where upper(CONTENT) like upper( ? ) or upper(TOPIC) like upper(?)";
		conn = DBConnection.getDBConnection();
		try{
			searchKey = "%"+searchKey+"%";
			pstmt = conn.prepareStatement(query);
		...
		finally
		{
			...
			{
				logger.severe("DAOException occured in getDocumentId" + "Exception Message: " + e.getMessage());
				throw new LMSException(e.getMessage(),e);
			}
		}	
	...
	public ArrayList<HashMap<String, String>> getCsrBPDetails(ArrayList<String> documentIds) throws DAOException{
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query="Select TOPIC,HEADER,CONTENT,FROM_DT,document_id from LMS.KM_BP_DATA where document_Id=?";
		...
		finally
		{
			...
			{
				logger.severe("DAOException occured in getCsrBPDetails" + "Exception Message: " + e.getMessage());
				throw new LMSException(e.getMessage(),e);
			}
		}	
	...
	public String[] getCircleList(String documentId) throws DAOException{
		...
		String query = "select distinct circle_id from LMS.KM_BP_DATA where DOCUMENT_ID=?";
		...
		finally
		{
			...
			{
				logger.severe("DAOException occured in getCircleList" + "Exception Message: " + e.getMessage());
				throw new LMSException(e.getMessage(),e);
			}
		}	
