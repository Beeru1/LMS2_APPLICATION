...
protected static final String SQL_INSERT_CSD_USER= " INSERT INTO LMS.KM_CSD_USERS(CSD_MOBILE_NO, CIRCLE_ID, STATUS, UPDATED_DATE) VALUES( ? , ? , 'A' , current date)";
protected static final String SQL_GET_CSD_USERS= " select CSD_MOBILE_NO from LMS.KM_CSD_USERS where CIRCLE_ID= ? and STATUS= ?  WITH UR";
protected static final String SQL_REMOVE_CSD_USER="UPDATE LMS.KM_CSD_USERS SET STATUS= ?,UPDATED_DATE= current date WHERE CIRCLE_ID= ? AND CSD_MOBILE_NO in ";	
//protected static final String SQL_GET_CIRCLE_NAME="SELECT em.ELEMENT_NAME FROM LMS.KM_CSD_USERS cu, LMS.KM_ELEMENT_MSTR em WHERE em.ELEMENT_ID=cu.CIRCLE_ID AND cu.STATUS= 'A' AND cu.CSD_MOBILE_NO= ?";
protected static final String SQL_GET_CIRCLE_COUNT="SELECT COUNT(*)FROM LMS.KM_CSD_USERS WHERE STATUS= 'A' AND CSD_MOBILE_NO= ?  WITH UR";

	public int insertCsd(ConfigCSDDto configCSDDto) throws DAOException {
		logger.info("Entered insert for table KM_CSD_USERS");
		////logger.info("inside dao impl insert function");
		Connection con = null;
	...
	public int getCircleCount(ConfigCSDDto configCSDDto) throws KmException {
		...
		try {
			...
			while(rs.next()){
				...
				//logger.info("circle name : "+rs.getInt(1));
			}
