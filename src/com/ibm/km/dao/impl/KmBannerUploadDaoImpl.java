...
public class KmBannerUploadDaoImpl implements KmBannerUploadDao{
	...
	.getLogger(KmBannerUploadDaoImpl.class.toString());

	public int uploadBanner(byte[] data,String fileName, String bannerPage) throws LMSException,DAOException{
		
		//logger.info("Inside dao impl uploadBanner() method!!!!");
		int result = 0;
		PreparedStatement pstmt = null;
		
		String insert_query = "insert into LMS.KM_IMAGE_MSTR values(?,?,?,?,?)";
		String update_query = "update LMS.KM_IMAGE_MSTR set FILE_NAME=?,IMAGE=?,TIME_STAMP=? where BANNER_PAGE=?";
		...
		finally
		{
			...
			{
				logger.severe("DAOException occured while uploadBanner" + "Exception Message: " + e.getMessage());
	...
	public InputStream retrieveBanner(String bannerPage) throws LMSException,DAOException{
		...
		//logger.info("Inside dao impl retrieveBanner() method!!!!");
		
		String query = "select * from LMS.KM_IMAGE_MSTR where BANNER_PAGE=? order by Time_Stamp desc FETCH FIRST 1 ROWS ONLY with ur";
		Connection conn = DBConnection.getDBConnection();
		PreparedStatement pstmt = null;
		...
		finally
		{
			...
			{
				logger.severe("DAOException occured while Approving File" + "Exception Message: " + e.getMessage());
				throw new LMSException(e.getMessage(),e);
			}
		}	
		return inputStream;
	...
	private boolean checkForBannerImageInDB(String bannerPage) throws DAOException{
		...
		Connection conn = DBConnection.getDBConnection();
		//logger.info("Inside checkForBannerImageInDB method!!!!");
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String select_query = "select * from LMS.KM_IMAGE_MSTR where BANNER_PAGE=? with ur";
		try{
		...
		finally
		{
			...
			{
				logger.severe("DAOException occured while checkForBannerImageInDB" + "Exception Message: " + e.getMessage());
				throw new LMSException(e.getMessage(),e);
			}
		}	
