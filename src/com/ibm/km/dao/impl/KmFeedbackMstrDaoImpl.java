...
public class KmFeedbackMstrDaoImpl implements KmFeedbackMstrDao{
	...
	protected static final String SQL_INSERT_WITH_ID ="INSERT INTO LMS.KM_FEEDBACK_MSTR (FEEDBACK_ID, COMMENT, CREATED_DT, CREATED_BY, CIRCLE_ID, CATEGORY_ID, SUB_CATEGORY_ID, ELEMENT_ID, READ_STATUS) VALUES ( NEXTVAL FOR LMS.KM_FEEDBACK_ID,?, current timestamp, ?, ?, 436, 440,?,'N')";
	
	public static final String SQL_UPDATE_STATUS = "UPDATE LMS.KM_FEEDBACK_MSTR SET READ_STATUS =?,FEEDBACK_RESP=?, UPDATED_DT=current timestamp WHERE FEEDBACK_ID = ?";
	
	protected static final String SQL_SELECT_FEEDBACKS ="WITH nee(element_id,chain,element_level_id,element_name) AS  ( SELECT  ELEMENT_ID, element_name,element_level_id,element_name  FROM LMS.KM_ELEMENT_MSTR  WHERE element_id = ?  UNION ALL   SELECT nplus1.element_id,nee.chain || '/' || nplus1.element_name,nplus1.element_level_id,nplus1.element_name  FROM LMS.KM_ELEMENT_MSTR as nplus1, nee  WHERE nee.element_id=nplus1.PARENT_ID) SELECT  fed.FEEDBACK_ID, chain,fed.COMMENT, fed.ELEMENT_ID, nee.ELEMENT_NAME, fed.CREATED_BY, fed.CREATED_DT, usr.USER_LOGIN_ID FROM LMS.KM_FEEDBACK_MSTR fed inner join NEE  on fed.ELEMENT_ID=nee.ELEMENT_ID inner join LMS.KM_USER_MSTR usr on usr.USER_ID=fed.CREATED_BY WHERE  fed.READ_STATUS='N'  ORDER BY fed.CREATED_DT  ";
	
   
   protected static final String SQL_FEEDBACK_RESP="SELECT COMMENT , CREATED_DT, UPDATED_DT,READ_STATUS,FEEDBACK_RESP FROM LMS.KM_FEEDBACK_MSTR WHERE CREATED_BY=?   ORDER BY UPDATED_DT DESC ";
   
   protected static final String SQL_FEEDBACK_REPORT ="WITH nee(element_id,chain,element_level_id,element_name) AS  ( SELECT  ELEMENT_ID, element_name,element_level_id,element_name  FROM LMS.KM_ELEMENT_MSTR  WHERE element_id = ?  UNION ALL   SELECT nplus1.element_id,nee.chain || '/' || nplus1.element_name,nplus1.element_level_id,nplus1.element_name FROM LMS.KM_ELEMENT_MSTR as nplus1, nee  WHERE nee.element_id=nplus1.PARENT_ID) SELECT fed.CREATED_DT,fed.UPDATED_DT,usr.USER_LOGIN_ID, fed.COMMENT, fed.READ_STATUS FROM LMS.KM_FEEDBACK_MSTR fed inner join NEE  on fed.ELEMENT_ID=nee.ELEMENT_ID inner join LMS.KM_USER_MSTR usr on usr.USER_ID=fed.CREATED_BY WHERE fed.READ_STATUS !='N'   and date(fed.CREATED_DT) between DATE(timestamp_format(?, 'YYYY-MM-DD HH24:MI:SS')) and DATE(timestamp_format(?, 'YYYY-MM-DD HH24:MI:SS'))  ORDER BY fed.CREATED_DT";
	/* (non-Javadoc)
	 * @see com.ibm.km.dao.KmFeedbackMstrDao#insert(com.ibm.km.dto.KmFeedbackMstr)
	 */
	public void insert(KmFeedbackMstr dto) throws LMSException {
	...
	public void readFeedbacks(String[] readFeedbacks,String[] feedbackResp, String[] feedBackId) throws DAOException {
		...
		{
			logger.error("KM Exception occured while reading feedback." + "Exception Message: " + km.getMessage());
		throw new LMSException("KM Exception: " + km.getMessage(),km);
		}
		catch(SQLException e)
		{
	...
	public ArrayList feedbackResponseAll(int userId) throws DAOException {
		...
		//for CircleId
		//String sql = "SELECT COMMENT , CREATED_DT, UPDATED_DT,READ_STATUS,FEEDBACK_RESP FROM LMS.KM_FEEDBACK_MSTR WHERE ELEMENT_ID in (select element_id from LMS.KM_ELEMENT_MSTR where PARENT_ID=?) AND UPDATED_DT IS NOT NULL ORDER BY UPDATED_DT DESC ";
		//or
		// for LOBId
		//String sql = "SELECT COMMENT , CREATED_DT, UPDATED_DT,READ_STATUS,FEEDBACK_RESP FROM LMS.KM_FEEDBACK_MSTR WHERE ELEMENT_ID in (select element_id from LMS.KM_ELEMENT_MSTR where PARENT_ID in (select element_id from LMS.KM_ELEMENT_MSTR where PARENT_ID=?)) ORDER BY UPDATED_DT DESC ";
		String sql = "SELECT COMMENT , CREATED_DT, UPDATED_DT,READ_STATUS,FEEDBACK_RESP FROM LMS.KM_FEEDBACK_MSTR WHERE CIRCLE_ID=?";
		try
		{
			con = DBConnection.getDBConnection();
	...
	public ArrayList feedbackResponse(int userId) throws DAOException {
		...
		{
			logger.error("KM Exception occured while reading feedback response." + "Exception Message: " + km.getMessage());
		throw new LMSException("KM Exception: " + km.getMessage(),km);
		}
		catch(SQLException e)
		{
