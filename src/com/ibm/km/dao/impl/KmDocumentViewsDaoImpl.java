...
public class KmDocumentViewsDaoImpl implements KmDocumentViewsDao {
	...
	private static Logger logger = Logger.getLogger(KmDocumentViewsDaoImpl.class.toString());
	// Code change due to the UAT observation
	protected static final String SQL_GET_VERSION_COUNT="select count(document_id)as VERSION_COUNT from LMS.KM_Document_mstr where element_id in(select element_id from LMS.KM_DOCUMENT_MSTR where document_id = ? ) and  approval_status = 'O'";
	...
	private Connection getConnection() throws LMSException {
		...
		try {
			...
			throw new LMSException("Exception while trying to obtain a connection in KmFavoritesDaoImpl",e);
		}
   }		
	
   public List showDocumentViews(String elementId,String fromDate,String toDate) throws LMSException {
		Connection con = null;
		...
		try {
			...
			/*	ps1=con.prepareStatement(SQL_GET_VERSION_COUNT);
				...
				//logger.info("DOctype="+rs.getInt("DOC_TYPE"));
