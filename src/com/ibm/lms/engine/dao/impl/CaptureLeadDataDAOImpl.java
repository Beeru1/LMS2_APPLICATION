...
//	    static final Logger leadLog = Logger.getLogger("LeadLogger");
	...
	//private static final String SQL_CHECK_PRODUCT="select PRODUCT_ID from PRODUCT_SYNONYM where ucase(PRODUCT_SYNONYM_NAME) = ? with ur ";
	/* Modified by Parnika for change that only products with active status should be considered */
	
	private static final String SQL_CHECK_PRODUCT="select pm.PRODUCT_ID from PRODUCT_MSTR pm , PRODUCT_SYNONYM ps where pm.PRODUCT_ID = ps.PRODUCT_ID and pm.STATUS = 'A' and ucase(PRODUCT_SYNONYM_NAME) = ? with ur"; 
	...
	private static final String SQL_GET_MULTIPLE_LEAD="select PROSPECT_ID from LEAD_PROSPECT_CUSTOMER where PROSPECT_MOBILE_NUMBER = ? with ur ";
	...
	private static final String SELECT_LEAD_PROSPECT_ID="Select LEAD_PROSPECT_ID from LEAD_PROSPECT_DETAIL where PROSPECT_ID=? and PRODUCT_LOB_ID=? with ur ";//Added by neetika on 20 nov	
	 */
	private static final String SELECT_LEAD_PROSPECT_ID="Select LEAD_PROSPECT_ID from LEAD_PROSPECT_DETAIL where LEAD_ID=? with ur ";//Added by neetika on 20 nov
	...
	private static final String GET_CIRCLE_ID="select CIRCLE_ID from CIRCLE_MSTR  where ucase(CIRCLE_NAME)=? AND LOB_ID=? with ur";  // changed this query Neetika
	...
	private static final String GET_Source_ID="select SOURCE_ID from SOURCE_MSTR  where ucase(SOURCE_NAME)=? with ur";
...
*/	
	...
	private static final String SELECT_AUTO_ASSIGN_LEAD = "SELECT * FROM AUTO_ASSIGNMENT_MATRIX WHERE LOB_ID = ? AND PRODUCT_ID = ? AND CIRCLE_ID = ? AND CITY_CODE = ? AND STATUS = 'A' WITH UR";
	
	private static final String SQL_GET_MULTIPLE_LEAD_FROM_DETAIL = "SELECT LEAD_PROSPECT_ID FROM LEAD_PROSPECT_DETAIL WHERE PROSPECT_ID = ? AND PRODUCT_LOB_ID = (SELECT PRODUCT_LOB_ID FROM PRODUCT_MSTR WHERE PRODUCT_ID = ? ) WITH UR";
	...
	private static final String UPDATE_LEAD_PROSPECT_CUSTOMER_EMAIL ="UPDATE LEAD_PROSPECT_CUSTOMER SET EMAIL =?  WHERE PROSPECT_ID =? WITH UR ";
	
	private static final String FIND_MAX_LEAD_CAPTURED_ID =" SELECT MAX(LEAD_CAPTURED_DATA_ID) AS LEAD_CAPTURED_DATA_ID  FROM LEAD_CAPTURE WITH UR";
	private static final String INSERT_LEAD_CAPTURED_DATA = "INSERT INTO LEAD_CAPTURE(PROSPECT_NAME, PROSPECT_MOBILE_NUMBER, ALTERNATE_CONTACT_NUMBER, LANDLINE_NUMBER, ADDRESS1, ADDRESS2, CITY, PINCODE, STATE, CIRCLE, MARITAL_STATUS, SOURCE, SUB_SOURCE, REQUEST_CATEGORY, APPOINTMENT_TIME, OPPORTUNITY_TIME, PRODUCT, EMAIL, PRIMARY_LANGUAGE, SUB_ZONE, ZONE, REMARKS, CAMPAIGN, REFER_PAGE, QUAL_LEAD_PARAM, GEO_IP_CITY, LEAD_SUBMIT_TIME, T_ID, F_ID, KEYWORD, FROM_PAGE, SERVICE, " +
			...
			" DOB,LC_ISDONE,EXTRA_PARAM1,EXTRA_PARAM2,EXTRA_PARAM3,EXTRA_PARAM4,EXTRA_PARAM5,EXTRA_PARAM6,EXTRA_PARAM7) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,'1',?,?,?,?,?,?,?) WITH UR ";
	public static final String INSERT_OTHER_LEAD_DETAILS = "INSERT INTO LEAD_DETAILS(LEAD_ID)VALUES(?) WITH UR";
	...
	public static final String SELECT_CREATE_LEAD_ID="select Max(LEAD_CAPTURED_DATA_ID) as LEAD_CAPTURED_DATA_ID from LEAD_CAPTURE WITH UR";
				...
				else
				{
					...
					try {
						...
						if (leadId != null && !leadId.equalsIgnoreCase("0") && smsFlag.equalsIgnoreCase("Y") && !autoAssign){
				    		...
				    		String SELECT_SMS_TEMPLATE = "SELECT MESSAGE_TEMPLATE FROM ALERT_MSTR  WHERE ALERT_ID =" +PropertyReader.getAppValue("lms.leadRegisteration.template")+ "  AND ACTIVE = 'A' WITH UR";
	...
	public AgencyResponseMessage captureLeadData(AgencyCaptureLeadDO[] leadData) throws DAOException
	{
		...
		logger.info("inside lead capture****************");
		Connection con = null;
		PreparedStatement ps = null;
		String responce ="";
	...
	public String  getLeadData(LeadCaptureServiceFirstVersDO leadData) throws DAOException {
		...
		logger.info("inside getleaddata");
		Connection con = null;
		PreparedStatement ps = null;
