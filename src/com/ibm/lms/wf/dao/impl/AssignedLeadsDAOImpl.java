...
/*	public static final String SQL_SELECT_ASSIGNED_LEADS ="SELECT LEAD.SUB_STATUS_ID, LEAD.LEAD_ID , LEAD.PRODUCT_ID, LEAD.LEAD_ASSIGNED_PRIMARY_USER,LEAD_ASSIGNMENT_TIME,EXPECTED_CLOSURE_DATE,PRO.PRODUCT_NAME FROM LEAD_TRANSACTION LEAD,PRODUCT_MSTR PRO,LEAD_DATA DATA WHERE LEAD.LEAD_STATUS_ID IN("+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.REASSIGNED+") AND DATA.LEAD_STATUS_ID = LEAD.LEAD_STATUS_ID AND LEAD.LEAD_ID=DATA.LEAD_ID AND PRO.PRODUCT_ID = DATA.PRODUCT_ID AND LEAD.SUB_STATUS_ID <> "+LMSStatusCodes.DIALER_CON_FAIL +" and " +
							"	LEAD.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION tr where  tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID IN("+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.REASSIGNED+")) AND " ;
*/
	/* Commented by Parnika
	// <changes made by Sudhanshu

	public static final String SQL_SELECT_ASSIGNED_LEADS ="SELECT LEAD.LEAD_ID, LSS.SUB_STATUS  , LEAD.PRODUCT_ID, LEAD.LEAD_ASSIGNED_PRIMARY_USER,LEAD_ASSIGNMENT_TIME,EXPECTED_CLOSURE_DATE,PRO.PRODUCT_NAME FROM LEAD_TRANSACTION LEAD,PRODUCT_MSTR PRO,LEAD_DATA DATA , LEAD_SUB_STATUS LSS WHERE LEAD.LEAD_STATUS_ID IN("+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.REASSIGNED+") AND DATA.LEAD_STATUS_ID = LEAD.LEAD_STATUS_ID AND LEAD.LEAD_ID=DATA.LEAD_ID AND PRO.PRODUCT_ID = DATA.PRODUCT_ID AND LEAD.SUB_STATUS_ID <> "+LMSStatusCodes.DIALER_CON_FAIL +" and " +
	"	LEAD.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION tr where  tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID) AND " ;

	// changes made by Sudhanshu>
		
	End of comments by Parnika */
	
	//QUERY changed by nancy Agrawall
	
	//public static final String SQL_SELECT_ASSIGNED_LEADS ="SELECT LEAD.LEAD_ID, LEAD.PRODUCT_ID, LEAD.LEAD_ASSIGNED_PRIMARY_USER,LEAD_ASSIGNMENT_TIME,EXPECTED_CLOSURE_DATE,PRO.PRODUCT_NAME ,(select  ls.SUB_STATUS  from LEAD_SUB_STATUS ls, LEAD_TRANSACTION tr where ls.SUB_STATUS_ID = tr.SUB_STATUS_ID and ls.PRODUCT_LOB_ID = tr.PRODUCT_ID  AND tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID and tr.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION t where  t.LEAD_ID = lead.LEAD_ID AND t.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID AND t.SUB_STATUS_ID NOT IN (380))),(select lss.SUB_SUB_STATUS from LEAD_SUB_SUB_STATUS lss , LEAD_SUB_STATUS LS,LEAD_TRANSACTION tr where  LS.UNIQUE_ID=(select  ls.UNIQUE_ID  from LEAD_SUB_STATUS ls, LEAD_TRANSACTION tr where ls.SUB_STATUS_ID = tr.SUB_STATUS_ID and ls.PRODUCT_LOB_ID = tr.PRODUCT_ID  AND tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID and tr.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION t where  t.LEAD_ID = lead.LEAD_ID AND t.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID AND t.SUB_STATUS_ID NOT IN (380))) AND ls.UNIQUE_ID=lss.SUBSTATUS_CODE AND  lss.SUB_SUB_STATUS_ID= tr.LEAD_SUB_SUB_STATUS_ID   AND tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID and tr.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION t where  t.LEAD_ID = lead.LEAD_ID AND t.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID AND t.SUB_STATUS_ID NOT IN (380)))   FROM LEAD_TRANSACTION LEAD,PRODUCT_MSTR PRO,LEAD_DATA DATA WHERE LEAD.LEAD_STATUS_ID IN (400,401 ) AND LEAD.SUB_STATUS_ID NOT IN (380) AND DATA.LEAD_STATUS_ID = LEAD.LEAD_STATUS_ID AND LEAD.LEAD_ID=DATA.LEAD_ID AND PRO.PRODUCT_ID = DATA.PRODUCT_ID AND LEAD.SUB_STATUS_ID <> 210   and LEAD.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION tr where  tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID AND tr.SUB_STATUS_ID NOT IN (210))"; 
	...
	public static final String SQL_SELECT_ASSIGNED_LEADS ="SELECT LEAD.LEAD_ID, LEAD.PRODUCT_ID, LEAD.LEAD_ASSIGNED_PRIMARY_USER,LEAD_ASSIGNMENT_TIME,EXPECTED_CLOSURE_DATE,PRO.PRODUCT_NAME , (select ls.SUB_STATUS  from LEAD_SUB_STATUS ls, LEAD_TRANSACTION tr where ls.SUB_STATUS_ID = tr.SUB_STATUS_ID and ls.PRODUCT_LOB_ID = tr.PRODUCT_ID  AND tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID and tr.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION t where  t.LEAD_ID = lead.LEAD_ID AND t.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID AND t.SUB_STATUS_ID NOT IN ("+LMSStatusCodes.FOLLOW_UP_DIALER_CALL+"))),(select lss.SUB_SUB_STATUS from LEAD_SUB_SUB_STATUS lss , LEAD_TRANSACTION tr,LEAD_SUB_STATUS LS where LS.UNIQUE_ID=(select  ls.UNIQUE_ID  from LEAD_SUB_STATUS ls, LEAD_TRANSACTION tr where ls.SUB_STATUS_ID = tr.SUB_STATUS_ID and ls.PRODUCT_LOB_ID = tr.PRODUCT_ID  AND tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID and tr.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION t where  t.LEAD_ID = lead.LEAD_ID AND t.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID AND t.SUB_STATUS_ID NOT IN (380))) AND ls.UNIQUE_ID=lss.SUBSTATUS_CODE and   lss.SUB_SUB_STATUS_ID= tr.LEAD_SUB_SUB_STATUS_ID   AND tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID and tr.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION t where  t.LEAD_ID = lead.LEAD_ID AND t.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID AND t.SUB_STATUS_ID NOT IN ("+LMSStatusCodes.FOLLOW_UP_DIALER_CALL+")))  " +
			"FROM LEAD_TRANSACTION LEAD,PRODUCT_MSTR PRO,LEAD_DATA DATA WHERE LEAD.LEAD_STATUS_ID IN ( "+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.REASSIGNED+" ) AND LEAD.SUB_STATUS_ID NOT IN ("+LMSStatusCodes.FOLLOW_UP_DIALER_CALL+") AND DATA.LEAD_STATUS_ID = LEAD.LEAD_STATUS_ID AND LEAD.LEAD_ID=DATA.LEAD_ID AND PRO.PRODUCT_ID = DATA.PRODUCT_ID AND LEAD.SUB_STATUS_ID <> "+LMSStatusCodes.DIALER_CON_FAIL +" " +
			" and LEAD.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION tr where  tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID AND tr.SUB_STATUS_ID NOT IN ("+LMSStatusCodes.FOLLOW_UP_DIALER_CALL+")) AND ";
	
	public static final String SQL_SELECT_ASSIGNED_LEADS1 = "SELECT LEAD.LEAD_ID , LEAD.PRODUCT_ID, LEAD.LEAD_ASSIGNED_PRIMARY_USER,LEAD_ASSIGNMENT_TIME,EXPECTED_CLOSURE_DATE,PRO.PRODUCT_NAME FROM LEAD_TRANSACTION LEAD,PRODUCT_MSTR PRO,LEAD_DATA DATA WHERE LEAD.LEAD_STATUS_ID IN("+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.REASSIGNED+") AND DATA.LEAD_STATUS_ID = LEAD.LEAD_STATUS_ID AND LEAD.LEAD_ID=DATA.LEAD_ID AND PRO.PRODUCT_ID = DATA.PRODUCT_ID AND LEAD.SUB_STATUS_ID <> "+LMSStatusCodes.DIALER_CON_FAIL +" AND LEAD.SUB_STATUS_ID IN ("+LMSStatusCodes.REASSIGNED+") AND " +
							" LEAD.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION tr where  tr.LEAD_ID = lead.LEAD_ID) AND " ;
	...
	//aman
		...
		/*public static final String SQL_SELECT_ASSIGNED_LEADS_ESCALATION ="SELECT LEAD.LEAD_ID, LEAD.PRODUCT_ID, LEAD.LEAD_ASSIGNED_PRIMARY_USER,LEAD_ASSIGNMENT_TIME,EXPECTED_CLOSURE_DATE,PRO.PRODUCT_NAME , (select SUB_STATUS from  LEAD_SUB_STATUS ls , LEAD_TRANSACTION tr where ls.SUB_STATUS_ID = tr.SUB_STATUS_ID AND tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID and tr.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION t where  t.LEAD_ID = lead.LEAD_ID AND t.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID)) " +
		"FROM LEAD_TRANSACTION LEAD,PRODUCT_MSTR PRO,LEAD_DATA DATA WHERE LEAD.LEAD_STATUS_ID IN ( "+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.ESCALATION+") AND DATA.LEAD_STATUS_ID = LEAD.LEAD_STATUS_ID AND LEAD.LEAD_ID=DATA.LEAD_ID AND PRO.PRODUCT_ID = DATA.PRODUCT_ID AND LEAD.SUB_STATUS_ID IN ("+LMSStatusCodes.ESCALATION1 +","+LMSStatusCodes.ESCALATION2+","+LMSStatusCodes.ESCALATION3+","+LMSStatusCodes.ESCALATION4+") " +
		" and LEAD.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION tr where  tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID)  ";
*/
	
	//query changed for assignment escalation
	//query changed for lobwise sub status
...
"AND PRO.PRODUCT_ID = DATA.PRODUCT_ID AND DATA.LEAD_STATUS_ID IN (" + LMSStatusCodes.ASSIGNED + " ," + LMSStatusCodes.REASSIGNED +" )";
	...
	/*public static final String SQL_SELECT_ASSIGNED_LEADS_FEASIBILITY ="SELECT LEAD.LEAD_ID, LEAD.PRODUCT_ID, LEAD.LEAD_ASSIGNED_PRIMARY_USER,LEAD_ASSIGNMENT_TIME,EXPECTED_CLOSURE_DATE,PRO.PRODUCT_NAME , (select SUB_STATUS from  LEAD_SUB_STATUS ls , LEAD_TRANSACTION tr where ls.SUB_STATUS_ID = tr.SUB_STATUS_ID AND tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID and tr.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION t where  t.LEAD_ID = lead.LEAD_ID AND t.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID)) " +
	"FROM LEAD_TRANSACTION LEAD,PRODUCT_MSTR PRO,LEAD_DATA DATA WHERE LEAD.LEAD_STATUS_ID IN ( "+LMSStatusCodes.FEASIBILITY+","+LMSStatusCodes.FEASIBILITY5+") AND DATA.LEAD_STATUS_ID = LEAD.LEAD_STATUS_ID AND LEAD.LEAD_ID=DATA.LEAD_ID AND PRO.PRODUCT_ID = DATA.PRODUCT_ID AND LEAD.SUB_STATUS_ID IN ("+LMSStatusCodes.FEASIBILITY1 +","+LMSStatusCodes.FEASIBILITY2+","+LMSStatusCodes.FEASIBILITY3+","+LMSStatusCodes.FEASIBILITY4+") " +
	" and LEAD.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION tr where  tr.LEAD_ID = lead.LEAD_ID AND tr.LEAD_STATUS_ID =DATA.LEAD_STATUS_ID)  ";
*/
	//query changed for lobwise sub status
		//changed query for feasibility escalation
...
//	public static final String SQL_USERS_LEAD_FORWARD ="SELECT USER_ID, USER_LOGIN_ID, USER_FNAME, USER_MNAME, USER_LNAME FROM USER_MSTR ";
	...
	//changed by sudhanshu >
/*	public static final String SQL_SELECT_VIEW_ASSIGNED_LEAD = " SELECT D.* ,(SELECT PRODUCT_NAME  FROM PRODUCT_MSTR where c.PRODUCT_ID=PRODUCT_ID) as PRODUCT_NAME	FROM ASSIGNMENT_MATRIX A,LEAD_PROSPECT_CUSTOMER B,LEAD_DATA C,LEAD_TRANSACTION D," +
		...
		"where 	A.OLM_ID =? AND	A.PRIMARY_AUTH=0 and A.status='A' AND A.CIRCLE_ID= B.CIRCLE_ID and a.RSU_ID=b.RSU_ID AND B.PROSPECT_ID=C.PROSPECT_ID AND C.LEAD_STATUS_ID IN ("+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.REASSIGNED+") AND " +
		"C.LEAD_ID=D.LEAD_ID AND D.LEAD_STATUS_ID IN("+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.REASSIGNED+") and c.LEAD_STATUS_ID=d.LEAD_STATUS_ID and (D.TRANSACTION_TIME) = (maxT.TRANSACTION_TIME) ";
	*/		
	/*
//<changed by sudhanshu 		
	public static final String SQL_SELECT_VIEW_ASSIGNED_LEAD = " SELECT D.* ,(SELECT PRODUCT_NAME  FROM PRODUCT_MSTR where c.PRODUCT_ID=PRODUCT_ID) as PRODUCT_NAME	FROM ASSIGNMENT_MATRIX A,LEAD_PROSPECT_CUSTOMER B,LEAD_DATA C,LEAD_TRANSACTION D,LEAD_PROSPECT_DETAIL LPD, " +
	" (select max(TRANSACTION_TIME) as TRANSACTION_TIME ,lead_id from LEAD_TRANSACTION TR where TR.lead_id = lead_id and LEAD_STATUS_ID IN("+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.REASSIGNED+")  group by lead_id) maxT " +
	"where 	A.OLM_ID =? AND	A.PRIMARY_AUTH=0 and A.status='A' AND A.CIRCLE_ID= LPD.CIRCLE_ID and a.RSU_ID=LPD.RSU_CODE AND B.PROSPECT_ID=C.PROSPECT_ID AND LPD.LEAD_PROSPECT_ID = c.LEAD_PROSPECT_ID and A.PRODUCT_LOB_ID = LPD.PRODUCT_LOB_ID AND C.LEAD_STATUS_ID IN ("+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.REASSIGNED+") AND " +
	"C.LEAD_ID=D.LEAD_ID AND D.LEAD_STATUS_ID IN("+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.REASSIGNED+") and c.LEAD_STATUS_ID=d.LEAD_STATUS_ID and (D.TRANSACTION_TIME) = (maxT.TRANSACTION_TIME) ";
	*/
	
	/* Modified By Parnika for removing duplicate Listing Of leads */		
	//query changed by Nancy Agrawal
...
//	public static final String SQL_LEAD_DETAIL  = "SELECT (SELECT  PRODUCT_NAME FROM PRODUCT_MSTR where PRODUCT_ID = a.PRODUCT_ID)as PRODUCT_NAME,a.LEAD_STATUS_ID ,(SELECT LEAD_STATUS FROM LEAD_STATUS WHERE LEAD_STATUS_ID =a.LEAD_STATUS_ID) AS LEAD_STATUS,(SELECT SUB_STATUS FROM LEAD_SUB_STATUS WHERE LEAD_STATUS_ID =a.LEAD_STATUS_ID AND SUB_STATUS_ID =a.LEAD_SUB_STATUS_ID) AS SUB_STATUS ,a.LEAD_ID,a.CROSSSELL_REF_LEAD_ID, " +
//			"b.CUSTOMER_NAME,lpd.RSU_CODE,(SELECT  CITY_NAME FROM CITY_MSTR where CITY_CODE=lpd.CITY_CODE) as CITY_NAME,(SELECT  STATE_NAME FROM STATE_MSTR where STATE_ID =lpd.STATE) as STATE_NAME, " +
//			" (SELECT LEAD_PRIORITY_NAME FROM LEAD_PRIORITY_SCORE_MAPPING WHERE LEAD_PRIORITY_VALUE = A.LEAD_PRIORITY ), " +
//			"(SELECT ZONE_NAME FROM ZONE_MSTR where ZONE_CODE=lpd.ZONE_CODE) as ZONE_NAME ,b.EMAIL,b.PROSPECT_MOBILE_NUMBER, cm.CIRCLE_NAME ,lpd.PINCODE,lpd.ALTERNATE_CONTACT_NUMBER, lpd.LANDLINE_NUMBER,LPD.ADDRESS1, lpd.COMPANY " +
//			" ,(SELECT CITY_ZONE_NAME FROM CITY_ZONE_MSTR where CITY_ZONE_CODE=lpd.CITY_ZONE_CODE) as CITY_ZONE_NAME , lpd.ZONE_CODE, lpd.CITY_CODE, lpd.CITY_ZONE_CODE, lpd.CIRCLE_ID" +
//			" ,lpd.RENTAL, A.ONLINE_CAF_NO,A.ALLOCATED_NO, LPD.MYOP_ID, LPD.PYT_AMT, LPD.TRAN_REFNO, pm.PRODUCT_LOB_ID ,LPD.GEO_IP_CITY , (Select LEAD_CATEGORY from LEAD_CATEGORY_MSTR where LEAD_CATEGORY_ID=A.LEAD_CATEGORY and STATUS='A' ) as LEAD_CATEGORY " +//changed by sudhanshu
//			" ,(SELECT SOURCE_NAME FROM SOURCE_MSTR WHERE SOURCE_ID = a.SOURCE  AND STATUS ='A') AS SOURCE_NAME,(SELECT SUBSOURCE_NAME FROM SUB_SOURCE_MSTR WHERE char(SUBSOURCE_ID) = a.SUB_SOURCE AND  STATUS ='A') SUBSOURCE_NAME ,A.UTM_CAMPAIGN , (SELECT PRODUCT_NAME FROM PRODUCT_MSTR where char(PRODUCT_ID) = a.REQUEST_TYPE)AS REQUEST_TYPE FROM 	LEAD_DATA 	a, LEAD_PROSPECT_CUSTOMER 	b, LEAD_PROSPECT_DETAIL    lpd, PRODUCT_MSTR pm, CIRCLE_MSTR  cm " +
...
//	
	//query changed by Nancy Agrawal for lobwise sub status
...
/*	public static final String SQL_LEAD_DETAIL  = "SELECT (SELECT  PRODUCT_NAME FROM PRODUCT_MSTR where PRODUCT_ID = a.PRODUCT_ID)as PRODUCT_NAME,a.LEAD_STATUS_ID,a.LEAD_ID,a.CROSSSELL_REF_LEAD_ID,b.CUSTOMER_NAME,b.RSU_ID,(SELECT  CITY_NAME FROM CITY_MSTR where CITY_CODE=b.CITY) as CITY_NAME," +
	"(SELECT  STATE_NAME FROM STATE_MSTR where STATE_ID=b.STATE) as STATE_NAME,( SELECT ZONE_NAME FROM ZONE_MSTR where ZONE_CODE=b.ZONE) as ZONE_NAME , b.EMAIL,b.PROSPECT_MOBILE_NUMBER, (SELECT CIRCLE_NAME FROM CIRCLE_MSTR where CIRCLE_ID=b.CIRCLE_ID) as CIRCLE_NAME , b.PINCODE, b.ALTERNATE_CONTACT_NUMBER, b.LANDLINE_NUMBER,b.ADDRESS1, b.COMPANY FROM LEAD_DATA a,LEAD_PROSPECT_CUSTOMER b where a.LEAD_ID=? and a.PROSPECT_ID=b.PROSPECT_ID ";
*/
	
/*	public static final String SQL_SELECT_ASSIGNED_LEAD_EXCEL = "SELECT PRO.PRODUCT_NAME,LT.LEAD_ID,LP.CUSTOMER_NAME,LP.ADDRESS1,LP.PROSPECT_MOBILE_NUMBER,LT.LEAD_ASSIGNMENT_TIME," +
	"LT.EXPECTED_CLOSURE_DATE from LEAD_DATA LD,LEAD_PROSPECT_CUSTOMER LP,LEAD_TRANSACTION LT,PRODUCT_MSTR PRO WHERE" +
	" LD.PROSPECT_ID = LP.PROSPECT_ID and LD.LEAD_STATUS_ID IN ("+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.REASSIGNED+") AND LT.SUB_STATUS_ID <> "+LMSStatusCodes.DIALER_CON_FAIL +" AND LT.LEAD_ID = LD.LEAD_ID AND LT.LEAD_STATUS_ID = LD.LEAD_STATUS_ID AND LD.PRODUCT_ID = PRO.PRODUCT_ID AND LT.LEAD_ASSIGNED_PRIMARY_USER = ? ";
	*/
	/* Commented by Parnika
	...
	" ,LPD.RENTAL, LD.ONLINE_CAF_NO,LD.ALLOCATED_NO, LPD.MYOP_ID, LPD.PYT_AMT, LPD.TRAN_REFNO,LPD.GEO_IP_CITY , (Select LEAD_CATEGORY from LEAD_CATEGORY_MSTR where LEAD_CATEGORY_ID=LD.LEAD_CATEGORY and STATUS='A' ) as LEAD_CATEGORY 	" +
	" from LEAD_DATA LD,LEAD_PROSPECT_CUSTOMER LP,LEAD_TRANSACTION LT,PRODUCT_MSTR PRO , LEAD_PROSPECT_DETAIL LPD WHERE" +
	" LD.PROSPECT_ID = LP.PROSPECT_ID AND LD.LEAD_PROSPECT_ID= lpd.LEAD_PROSPECT_ID and LD.LEAD_STATUS_ID IN ("+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.REASSIGNED+") AND LT.SUB_STATUS_ID <> "+LMSStatusCodes.DIALER_CON_FAIL +" AND LT.LEAD_ID = LD.LEAD_ID AND LT.LEAD_STATUS_ID = LD.LEAD_STATUS_ID AND LD.PRODUCT_ID = PRO.PRODUCT_ID AND LT.LEAD_ASSIGNED_PRIMARY_USER = ? " +
	...
	/* Modified By Parnika for LMS Phase 2 drop 3 */
	
//	public static final String SQL_SELECT_ASSIGNED_LEAD_EXCEL = "SELECT PRO.PRODUCT_NAME,LT.LEAD_ID,LP.CUSTOMER_NAME,LPD.ADDRESS1,LP.PROSPECT_MOBILE_NUMBER,LT.LEAD_ASSIGNMENT_TIME," +
//	"LT.EXPECTED_CLOSURE_DATE,  (SELECT ZONE_NAME FROM ZONE_MSTR where ZONE_CODE=lpd.ZONE_CODE) as ZONE_NAME  , (SELECT LEAD_PRIORITY_NAME FROM LEAD_PRIORITY_SCORE_MAPPING WHERE LEAD_PRIORITY_VALUE = LD.LEAD_PRIORITY ), (SELECT CITY_NAME FROM CITY_MSTR WHERE CITY_CODE = LPD.CITY_CODE ),(SELECT CITY_ZONE_NAME FROM CITY_ZONE_MSTR where CITY_ZONE_CODE=lpd.CITY_ZONE_CODE) as CITY_ZONE_NAME  " +
//	" ,LPD.RENTAL, LD.ONLINE_CAF_NO,LD.ALLOCATED_NO, LPD.MYOP_ID, LPD.PYT_AMT, LPD.TRAN_REFNO,LPD.EXTRA_PARAMS_2, LPD.GEO_IP_CITY , (Select LEAD_CATEGORY from LEAD_CATEGORY_MSTR where LEAD_CATEGORY_ID=LD.LEAD_CATEGORY and STATUS='A' ) as LEAD_CATEGORY ,(SELECT pr.PRODUCT_NAME as REQUEST_TYPE FROM PRODUCT_MSTR pr WHERE pr.PRODUCT_ID = LD.REQUEST_TYPE), LPD.PINCODE	" +
//	" from LEAD_DATA LD,LEAD_PROSPECT_CUSTOMER LP,LEAD_TRANSACTION LT,PRODUCT_MSTR PRO , LEAD_PROSPECT_DETAIL LPD WHERE" +
//	" LD.PROSPECT_ID = LP.PROSPECT_ID AND LD.LEAD_PROSPECT_ID= lpd.LEAD_PROSPECT_ID and LD.LEAD_STATUS_ID IN ("+LMSStatusCodes.ASSIGNED+","+LMSStatusCodes.REASSIGNED+") AND LT.SUB_STATUS_ID <> "+LMSStatusCodes.DIALER_CON_FAIL +" AND LT.LEAD_ID = LD.LEAD_ID AND LT.LEAD_STATUS_ID = LD.LEAD_STATUS_ID AND LD.PRODUCT_ID = PRO.PRODUCT_ID AND LT.LEAD_ASSIGNED_PRIMARY_USER = ? " +
//	"  AND  LT.TRANSACTION_TIME = (select max(TRANSACTION_TIME) FROM LEAD_TRANSACTION tr where  tr.LEAD_ID = LT.LEAD_ID AND tr.LEAD_STATUS_ID =LD.LEAD_STATUS_ID AND tr.LEAD_ASSIGNED_PRIMARY_USER = ? ) ";
//	
...
/*	public static final String SQL_SELECT_CIRCLE_COORDINATOR ="SELECT UM.USER_ID, UM.USER_LOGIN_ID, UM.USER_FNAME, UM.USER_MNAME, UM.USER_LNAME FROM USER_MSTR UM WHERE KM_ACTOR_ID = "+Constants.DOC_TYPE_SOP+" AND UM.CIRCLE_ID = ? ";
	*/
	//<changed by sudhanshu
	...
	public ArrayList<Leads> listAssignedLeads(String loginID,String startDate,String endDate,String view) throws LMSException {
		...
		try {
			...
			*/
			...
			ps1 = con.prepareStatement(query.append(" ORDER BY LEAD_ID with ur ").toString());
			...
			//logger.info("SQL Stmt query0 :" +query);
			while (rs1.next()) {
				dto = new Leads();
			...
			//ArrayList<Leads> assignedList = new ArrayList<Leads>();
		//	con = DBConnection.getDBConnection();
			ps2 = con.prepareStatement(query1.append(" ORDER BY LEAD_ID with ur ").toString());
		...
		//logger.info("SQL Stmt query1 :" +query1);
			while (rs2.next()) {
				dto = new Leads();
			...
			//ArrayList<Leads> assignedList = new ArrayList<Leads>();
		//	con = DBConnection.getDBConnection();
			ps3 = con.prepareStatement(query2.append(" ORDER BY LEAD_ID with ur ").toString());
			...
			//logger.info("SQL Stmt query2 :" +query2);
			while (rs3.next()) {
				dto = new Leads();
	...
	public ArrayList<Constant> getActionList(String keyName) throws LMSException {
		...
		try {
			...
			con = DBConnection.getDBConnection();
			ps = con.prepareStatement(query.append("ORDER By LEAD_STATUS with ur ").toString());
...
/*			
	...
	public Boolean closeTheLead(ArrayList masterList) throws LMSException {
		...
		try {
			...
			/* End of changes by Parnika */
			ps1 = con.prepareStatement(query.append(" with ur ").toString());
			...
			logger.info("lead status updated success");
				...
				dmlStatement.append(" ORDER BY TRANSACTION_TIME DESC FETCH FIRST ROW ONLY WITH UR ");
	...
	public Boolean reAssignTheLead(LeadForm commonForm) throws LMSException {
		...
		try {
			...
			if(count == 1)
			{
				...
				ps2.setInt(counter++ , LMSStatusCodes.DIALER_SECOND_CALL);
				//logger.info(dmlStatement);
				count = ps2.executeUpdate();
				if(count == 1)
				flag = true;
				
				/*ps2=con.prepareStatement("select USER_MOBILE_NUMBER from USER_MSTR where USER_LOGIN_ID=? with UR");
	...
	return rentalStr.toString();
	 ...
	 private String getRemarksLatest( long leadId) throws DAOException{
			...
			try {
				...
				pst = connection.prepareStatement(query.append(" with ur ").toString());		
	...
	public ArrayList<UserDto> getChannelPartnerListForLead(ArrayList circleList, LeadDetailDTO detailDTO) throws LMSException {
		...
		try {
			...
			if (detailDTO.getZoneCode() != null && !detailDTO.getZoneCode().equals("")) {
				StringBuffer query1=new StringBuffer("SELECT ZONE_CODE, ZONE_NAME FROM ZONE_MSTR where ucase(zone_name)='DUMMY' and ZONE_CODE = ?  WITH UR ");														
	...
	public ArrayList<UserDto> getChannelPartnerListForLeadImproved(ArrayList circleList, LeadDetailDTO detailDTO,ArrayList lobList) throws LMSException {
		...
		try {
			...
			if (detailDTO.getZoneCode() != null && !detailDTO.getZoneCode().trim().equals("")) {
				...
				query1.append("SELECT ZONE_CODE, ZONE_NAME FROM ZONE_MSTR where ucase(zone_name)='DUMMY' and ZONE_CODE = ?  WITH UR ");														
				ps1 = con.prepareStatement(query1.toString());
				ps1.setString(1, detailDTO.getZoneCode());
				rs1 = ps1.executeQuery();
				...
				if	(((detailDTO.getZoneCode() == null || detailDTO.getZoneCode().equals("")) && (detailDTO.getCityZoneCode() == null || detailDTO.getCityZoneCode().equals("")))///both zone and cityZone are not available in the lead
					 ...
					 ps = con.prepareStatement(listOfUsers.append(" with ur ").toString());
						rs = ps.executeQuery();
						while (rs.next()) {
							dto = new UserDto();
							dto.setUserLoginId(rs.getString("USER_LOGIN_ID"));
				...
				ps2 = con.prepareStatement(getCircleCoordinators.append(" with ur ").toString());
					rs2 = ps2.executeQuery();
					while (rs2.next()) {
						dto = new UserDto();
...
public ArrayList<UserDto> getUserListForNewProduct(LeadDetailDTO leadDetails,String userLoginId) throws LMSException
 	{
		...
		try {
			ArrayList<UserDto> masterList = new ArrayList<UserDto>();
			con = DBConnection.getDBConnection();
			ps = con.prepareStatement("select unique um.USER_LOGIN_ID,upc.AGENT_NAME from USER_MSTR um,NEW_PROD_CONFIG upc where um.USER_LOGIN_ID=upc.OLMID and upc.CIRCLE_ID=? and LOB=? and um.STATUS='A' and um.USER_LOGIN_ID <> ? with ur");
...
/*			
 ...
 public int getLeadStatus(long leadId) throws Exception{
	 ...
	 try{
		 con=DBConnection.getDBConnection();
		 ps=con.prepareStatement("SELECT LEAD_STATUS_ID FROM LEAD_DATA WHERE LEAD_ID=? with UR");
		 ps.setLong(1, leadId);
		 rs=ps.executeQuery();
		 if(rs.next()){
 ...
 public ArrayList<String> getSiteClosureList(String lob)  {
	 ...
	 try{
		 ...
		 ps=con.prepareStatement("SELECT OLMID FROM NEW_PROD_CONFIG NPF, PRODUCT_LOB PL WHERE NPF.LOB=PL.PRODUCT_LOB_ID AND PL.PRODUCT_LOB_ID IN ("+lob+") AND NPF.STATUS='S' WITH UR");
		 rs=ps.executeQuery();
 ...
 try{
		 ...
		 ps=con.prepareStatement("select LEAD_ASSIGNED_PRIMARY_USER from LEAD_TRANSACTION where lead_id="+leadId+" order by TRANSACTION_TIME desc fetch first row only WITH UR");
