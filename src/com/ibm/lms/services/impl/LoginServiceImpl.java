...
import com.ibm.lms.dao.PopulateUserDao;
import com.ibm.lms.dao.LoginDao;
import com.ibm.lms.dao.impl.LoginDaoImpl;
import com.ibm.lms.dao.impl.PopulateUserDaoImpl;
...
public class LoginServiceImpl implements LoginService {
	...
	public UserMstr populateUserDetails(Login login) throws LMSException, DAOException {
		...
		try {
			...
			////logger.info("Circle Flag: "+circleBean.getSingleSignInFlag()+" User Login Status: "+userBean.getUserLoginStatus());
	...
	private UserMstr setUserBeanData(List dataList, UserMstr userBean) throws LMSException {
		...
		try {
			...
			LOGGER.info("MODULEEE DTATTTT"+moduleList);
			moduleList = null;
	...
	public String getStringFromDate(java.util.Date d) {
		...
		public UserMstr populateUserDetailsforUD(String loginID) throws LMSException, DAOException {
			LOGGER.info("Populating user credentials" + loginID);
			PopulateUserDao populateDao = PopulateUserDaoImpl.populateUserDaoInstance();//changed by srikant new PopulateUserDaoImpl();
			UserMstr userBean = new UserMstr();
			CircleMstr circleBean = new CircleMstr();
			List dataList = null;
		...
		public String sendSMS(String userLoginId,String contact) throws LMSException {
			LOGGER.debug("sending sms to configurable users");
			LoginDao kmLoginDaoImpl = LoginDaoImpl.loginDaoInstance();
			return kmLoginDaoImpl.smsUserList(userLoginId,contact);
	        }
}
