...
private static ProductMappingDaoImpl productMappingDaoImpl=null;
	...
	public  ArrayList<ProductDTO> getProductList() throws DAOException
	{
		...
		try {
			...
			ps = con.prepareStatement("select PRODUCT_ID, PRODUCT_LOB_ID, PRODUCT_NAME, PRODUCT_DESC, PRODUCT_HEAD_OLM_ID from PRODUCT  with ur");
			rs = ps.executeQuery();
			while(rs.next()) {
				dto = new ProductDTO();
	...
	public  ArrayList<ProductDTO> getProductLobList() throws DAOException
	{
		...
		try {
			con = DBConnection.getDBConnection();
			ps = con.prepareStatement("SELECT PRODUCT_LOB, PRODUCT_LOB_ID FROM PRODUCT_LOB where STATUS='A'  WITH UR ");
			rs = ps.executeQuery();
			while(rs.next()) {
				dto = new ProductDTO();
				dto.setProductLobId(rs.getInt("PRODUCT_LOB_ID"));
	...
	public  ArrayList<ProductDTO> getProductList(int productLobId) throws DAOException
	{
		...
		try {
			con = DBConnection.getDBConnection();
			ps = con.prepareStatement("SELECT PRODUCT_ID, PRODUCT_NAME, PRODUCT_DESC, PRODUCT_HEAD_OLM_ID FROM PRODUCT_MSTR WHERE PRODUCT_LOB_ID = ? and STATUS='A' WITH UR");
	...
	public  ArrayList<StateDTO> getStateList() throws DAOException
	{
		...
		try {
			con = DBConnection.getDBConnection();
			ps = con.prepareStatement("select  STATE_ID,     STATE_NAME,     STATE_DESC  from STATE_MSTR with UR");
			rs = ps.executeQuery();
			while(rs.next()) {
				dto = new StateDTO();
	...
	public  ArrayList<CircleDTO> getCircleList() throws DAOException
	{
		...
		try {
			con = DBConnection.getDBConnection();
			ps = con.prepareStatement("select   CIRCLE_ID  ,   CIRCLE_NAME  ,   CIRCLE_DESC  from CIRCLE_MSTR with UR");
			rs = ps.executeQuery();
			while(rs.next()) {
				dto = new CircleDTO();
	...
	public int insertProductSynonym(ProductDTO productDto) throws DAOException
	{
		...
		try {
			con = DBConnection.getDBConnection();
			ps = con.prepareStatement("INSERT INTO PRODUCT_SYNONYM(PRODUCT_SYNONYM_NAME,PRODUCT_ID,STATUS, TRANSACTION_TIME, UPDATED_BY) VALUES(?,?,'A', current timestamp, ?) WITH UR");			
	...
	public  ArrayList<ProductDTO> getSynonymListBasedOnProduct(int selectedProductId) throws DAOException
	{
		...
		try {
			con = DBConnection.getDBConnection();
			ps = con.prepareStatement("select PRODUCT_SYNONYM_NAME,PRODUCT_SYNONYM_ID from PRODUCT_SYNONYM where PRODUCT_ID = ? and STATUS = 'A' with ur");
	...
	public String getDataForPinCode(String pinCode,int circleMstrId ,int productlobId) throws DAOException {
		logger.info(" in getDataForPinCode of productmappingdaoimpl :::::");
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		String cityCode="";
		...
		if(circleMstrId !=0 && productlobId==0) {
			...
			"zm.CIRCLE_MSTR_ID= cm.CIRCLE_MSTR_ID and pm.PINCODE=? AND cm.CIRCLE_MSTR_ID=? AND pm.STATUS='A' with UR";
		}else {
			 GET_ALL_DETAILS_ON_PINCODE="SELECT pm.CITY_ZONE_CODE,czm.CITY_ZONE_NAME, czm.CITY_CODE,ctm.CITY_NAME,ctm.ZONE_CODE,zm.ZONE_NAME,zm.CIRCLE_MSTR_ID FROM CIRCLE_MSTR CM,ZONE_MSTR ZM,CITY_MSTR CTM ,CITY_ZONE_MSTR CZM,PINCODE_MSTR PM "+
			 "WHERE  PM.CITY_ZONE_CODE=czm.CITY_ZONE_CODE AND czm.CITY_CODE=ctm.CITY_CODE AND ctm.ZONE_CODE= zm.ZONE_CODE AND "+
			"zm.CIRCLE_MSTR_ID= cm.CIRCLE_MSTR_ID and pm.PINCODE=? AND cm.LOB_ID=? AND pm.STATUS='A' with UR";
		}
			 ...
			 if (!pinCode.equalsIgnoreCase(""))
	    		{
				  con = DBConnection.getDBConnection();
	    			...
	    			return "";
	    			 ...
	    			 logger.info("circleMstrId != circleMstr_Id :"+" "+(circleMstrId != circleMstr_Id));
	    			 logger.info("cityZone_code :"+cityZone_code+"  cityzone_name :"+cityZone_name+" citycode :"+cityCode+" cityname :"+cityName+"  zonecode  :"+zone_code+" zone_name : "+zone_name);
	    		}
...
" AND ZM.ZONE_CODE=CTM.ZONE_CODE AND CTM.STATUS='A' AND CTM.CITY_CODE=CZM.CITY_CODE AND CZM.STATUS='A'";
		*/
		...
		try {
			logger.info("GET_ALL_DETAILS_FOR_CIRCLEMASTERID :"+GET_ALL_DETAILS_FOR_CIRCLEMASTERID+"  circleMstrId:"+circleMstrId);
