...
public class CircleManagementDaoImpl implements CircleManagementDao {

	Logger logger = Logger.getLogger(CircleManagementDaoImpl.class);
	
//	private final static String GET_CIRCLE_STATE_DETAILS = "select CIRCLE_MSTR_ID, State_name from STATE_MSTR where CIRCLE_MSTR_ID in (Select CIRCLE_MSTR_ID from CIRCLE_MSTR where LOB_ID = "+
//				"(Select product_lob_id from PRODUCT_MSTR where PRODUCT_NAME = ? )) with ur"; 
...
//Added by srikant 
	 ...
	 private static CircleManagementDaoImpl circleManagementDaoImpl=null;
		
		private CircleManagementDaoImpl(){
			
		}
		
		public static CircleManagementDaoImpl circleManagementDaoInstance()
		{
			if(circleManagementDaoImpl==null)
			{
				circleManagementDaoImpl=new CircleManagementDaoImpl();
			}
			return circleManagementDaoImpl;
...
//			
	...
	public RetrieveLeadDataDTO[] getLeadList(String leadId,String prospectMobileNumber, String transactionId, String flag)  {
		...
		List<RetrieveLeadDataDTO>  retrieveLeadDataDTOs =null;
		RetrieveLeadDataDTO[] dtos =null;
		...
		try {
			...
			con = DBConnection.getDBConnection();
			StringBuffer query=new StringBuffer(GET_LEAD_DATA);
			...
			if(lead !=null && lead >0) {
				...
				retrieveLeadDataDTOs = resultSetData(rsLeadData);
			}else if(prospectMobileNumber !=null && prospectMobileNumber.trim().length() >0){
		...
		if(!mobilFlag && flag !=null && flag.equalsIgnoreCase("H") && retrieveLeadDataDTOs != null && retrieveLeadDataDTOs.size() >0) 
		 {
			 ...
			 RetrieveLeadDataDTO dtoMain  = retrieveLeadDataDTOs.get(0);
			 ArrayList<RetrieveLeadDataDTO> list=  resultHistorySetData(con, dtoMain);
			 
			 if(list ==null || list.size()<=0)
			 {
				 RetrieveLeadDataDTO dataDTO  = new RetrieveLeadDataDTO();
				 dataDTO = dtoMain;
		 ...
		 dtos  = retrieveLeadDataDTOs.toArray(new RetrieveLeadDataDTO [retrieveLeadDataDTOs.size()]); //history list to array
			  }
	...
	//History data
	private ArrayList<RetrieveLeadDataDTO> resultHistorySetData(Connection con ,RetrieveLeadDataDTO dtoMain) throws DAOException
	{
		
		PreparedStatement psLeadProspectdetailH=null;
		ResultSet rsLeadProspectdetailH = null;
		RetrieveLeadDataDTO dto  = null;
		ArrayList<RetrieveLeadDataDTO> leadHstDetailsList = new ArrayList<RetrieveLeadDataDTO>();
	  ...
	  try {
			...
			while(rsLeadProspectdetailH.next()) 
			{
				...
				dto.setExtraParam1(rsLeadProspectdetailH.getString(ColumnKeys.EXTRA_PARAM1));
				dto.setExtraParam3(rsLeadProspectdetailH.getString(ColumnKeys.EXTRA_PARAM3));
				dto.setExtraParam4(rsLeadProspectdetailH.getString(ColumnKeys.EXTRA_PARAM4));
				dto.setExtraParam5(rsLeadProspectdetailH.getString(ColumnKeys.EXTRA_PARAM5));
				dto.setExtraParam6(rsLeadProspectdetailH.getString(ColumnKeys.EXTRA_PARAM6));
				dto.setExtraParam7(rsLeadProspectdetailH.getString(ColumnKeys.EXTRA_PARAM7));
				dto.setExtraParam8(rsLeadProspectdetailH.getString(ColumnKeys.EXTRA_PARAM8));
	...
	private List<RetrieveLeadDataDTO> resultSetData(ResultSet rs) {
		RetrieveLeadDataDTO dto =null;
		List<RetrieveLeadDataDTO> leadList=null;
		
		try {

			leadList=new ArrayList<RetrieveLeadDataDTO>();
			while(rs.next())
			{				
				dto=new RetrieveLeadDataDTO();
